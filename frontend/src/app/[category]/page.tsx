...
export default async function CategoryPage({ params }: { params: Promise<{ category: string }> }) {
  ...
  const {  data: posts } = await supabase // ✅ di sini perubahannya
    .from('posts')
    .select('*')
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6">{CATEGORY_MAP[category]}</h1>
      <div className="space-y-6">
        {posts?.length ? (
          posts.map(post => (
            <div key={post.id} className="border rounded-lg p-4 hover:bg-gray-50">
              <Link href={`/${category}/${post.slug}`} className="text-xl font-semibold text-blue-600 hover:underline">
                {post.title}
              </Link>
